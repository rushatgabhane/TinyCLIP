...
We replace multi-head attention (MHA) in a pretrained \textbf{TinyCLIP-8M} with grouped-query (GQA) and multi-query (MQA) attention in \emph{both} the image and text encoders, by mean-pooling each group's key/value heads, the conversion recipe of Ainslie et al. \cite{gqa2023}. Conversion alone reduces average zero-shot accuracy from 56.8\% to about 5\%, but a short feature-distillation uptrain recovers most of it: GQA-2 reaches 93\% of MHA accuracy on CIFAR-100 using half the K/V parameters. On these bidirectional encoders the only saving is parameter count; throughput and peak memory are unchanged, since there is no autoregressive KV-cache to reduce. The efficiency benefit of GQA and MQA therefore does not transfer to CLIP-style encoders.
...
\includegraphics[width=\linewidth]{attention_diagram_tinyclip-8m.png}
\captionof{figure}{\small\color{NavyBlue} All 4 query heads are kept; key/value heads are pooled into G groups: MHA (4), GQA-2 (2), MQA (1).}
\vspace{0.15cm}
\end{center}
...
\headerbox{Zero-shot accuracy: conversion and uptraining}
\vspace{0.4cm}
...
\headerbox{Efficiency: parameters, throughput, and memory}
\vspace{0.4cm}
...
\bottomrule
\end{tabular}
\captionof{table}{\small\color{NavyBlue} K/V projection parameters drop 2x/4x, but total params, throughput and peak memory are essentially flat.}
\end{center}
...
\captionof{figure}{\small\color{NavyBlue} Accuracy vs.\ cost. GQA-2 is the favorable point (half the K/V params, about 77\% accuracy); MQA is too aggressive for a 4-head model.}
\vspace{0.15cm}
\end{center}

\headerbox{Encoder vs.\ decoder: no KV-cache}
GQA and MQA reduce memory by shrinking the KV-cache during autoregressive decoding. A CLIP encoder is bidirectional and runs in a single pass, so it has no KV-cache. Ainslie et al.\ \cite{gqa2023} apply GQA only to the decoder, not the encoder. On CLIP the only saving is the K/V projection weights (a small fraction of the model), and broadcasting K/V back to full width leaves the attention computation unchanged.
...
\begin{itemize}\setlength\itemsep{0pt}\setlength\topsep{0pt}\setlength\parsep{0pt}\setlength\partopsep{0pt}
    \item Conversion alone reduces zero-shot accuracy to near-random on a small CLIP; a short uptrain recovers most of it.
    \item GQA-2 gives the best accuracy/compression trade-off: about 77\% of MHA accuracy overall and 93\% on CIFAR-100 at half the K/V parameters. MQA loses substantially more accuracy for 4x compression.
    \item On a bidirectional encoder the only benefit is parameter compression; there is no throughput or memory gain, because there is no KV-cache.
    \item GQA and MQA reduce the number of K/V parameters on CLIP encoders but do not improve inference speed or memory.
    \item Future work: larger TinyCLIP (more heads allow a finer GQA sweep), longer uptraining, and a vision-only vs.\ both-encoder ablation.
\end{itemize}
\color{Black}
